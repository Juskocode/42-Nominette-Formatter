...
def create_app(config=None):
    ...
    def api_project_history():
        """API endpoint to get project scan history."""
    ...
    def api_project_metrics():
        """API endpoint to get detailed project metrics."""
        if not dashboard.current_project:
        ...
        try:
            ...
            severity_metrics = {
                'critical': sum(f.critical_errors for f in files),
                'high': sum(f.high_errors for f in files),
                'medium': sum(f.medium_errors for f in files),
                'low': sum(f.low_errors for f in files)
            }

            # File size distribution
            size_distribution = {
                'small': len([f for f in files if f.lines_of_code < 50]),
                'medium': len([f for f in files if 50 <= f.lines_of_code < 200]),
                'large': len([f for f in files if f.lines_of_code >= 200])
            }

            # Error density (errors per line of code)
            total_loc = sum(f.lines_of_code for f in files)
            total_errors = sum(f.error_count for f in files)
            error_density = (total_errors / total_loc * 100) if total_loc > 0 else 0

            # Most problematic files
            problematic_files = dashboard.aggregator.get_most_problematic_files(5)

            return jsonify({
                'success': True,
                'metrics': {
                    'severity_distribution': severity_metrics,
                    ...
                    'most_problematic_files': [
                        {
                            ...
                            'error_density': round((f.error_count / f.lines_of_code * 100) if f.lines_of_code > 0 else 0, 2)
                        }
    ...
    @app.route('/api/files/batch-operations', methods=['POST'])
    def api_batch_operations():
        """API endpoint for batch operations on files."""
        data = request.get_json()
        operation = data.get('operation')
        filepaths = data.get('filepaths', [])

        if not operation or not filepaths:
            return jsonify({'success': False, 'error': 'Operation and filepaths are required'}), 400
        ...
        try:
            ...
            if operation == 'scan':
                ...
                for filepath in filepaths:
                    result = dashboard.scanner.scan_file(filepath)
                    analyses = []
                    if result.errors:
                        analyses = dashboard.parser.analyze_file_errors(result.errors)

                    dashboard.scan_results[filepath] = {
                        'result': result,
                        'analyses': analyses
                    }
                    dashboard.aggregator.add_scan_result(result, analyses)
                ...
                # Create backups for selected files
                for filepath in filepaths:
                    success = dashboard.formatter._create_backup(filepath)
                    results[filepath] = {
                        'success': success,
            ...
            return jsonify({
                ...
                'operation': operation,
                'results': results,
                'processed_count': len(filepaths)
            })

        except Exception as e:
            logger.error(f"Error in batch operation {operation}: {e}")
            return jsonify({'success': False, 'error': str(e)})
